  ...
  void submitDosage() {
    if (!formKey.currentState!.validate() ||
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Please fill all required fields and add at least one time")));
      return;
    }
    ...
    final dosageData = {
      ...
      'times': selectedTimes.map((t) => {'time': t, 'taken': false, 'takenDate': null}).toList(),
      'startDate': Timestamp.fromDate(DateTime.parse(StartDateController.text)),
      'endDate': hasEndDate ? Timestamp.fromDate(DateTime.parse(EndDateController.text)) : null,
      'addedAt': Timestamp.fromDate(DateTime.now()),
      'medicineId': selectedMedicine!.id,
    };

    // Dispatch AddDosageEvent to DosageBloc
    context.read<DosageBloc>().add(AddDosageEvent(userCredential?.uid ?? '', selectedMedicine!.id, dosageData));

    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Dosage added successfully!")));
            ...
            child: Center(
              child: Column(
                children: [
                  ...
                  const Icon(FontAwesomeIcons.pills, color: AppColors.primary, size: 80),
                  const SizedBox(height: 20),
                      ...
                      if (state is MedicineLoadingState) {
                        ...
                        return SizedBox(
                          ...
                          child: DropdownButtonFormField<Medicine>(
                            ...
                            items:
                                state.medicines
                                    .map((med) => DropdownMenuItem(value: med, child: Text(med.name)))
                                    .toList(),
                            onChanged: (val) {
                              ...
                              if (val != null) {
                                context.read<DosageBloc>().add(LoadDosagesEvent(userCredential?.uid ?? '', val.id));
                              }
                            },
                  ...
                  Row(
                    ...
                    children: [
                      Checkbox(value: hasEndDate, onChanged: (val) => setState(() => hasEndDate = val!)),
                  ...
                  Wrap(
                    ...
                    children:
                        selectedTimes
                            .map((t) => Chip(label: Text(t), onDeleted: () => setState(() => selectedTimes.remove(t))))
                            .toList(),
                  ),
                  const SizedBox(height: 20),
                  // Submit Button
                  SizedBox(
                    height: 50,
                    width: 300,
                    child: ElevatedButton(onPressed: submitDosage, child: const Text("Add Dosage")),
                  ),
                  const SizedBox(height: 20),
                ],
              ),
